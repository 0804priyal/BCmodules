define([
    ...
    return Component.extend({
        ...
        initialize: function () {
            ...
            o.options.sectionAPrice = o.first_line_text_price;
            o.options.sectionBPrice = o.instrument_name_price;
            o.options.sectionCPrice = o.die_imprint_price;
            o.options.diePrice      = o.die_creation_price;
            /*** Add Dynamic Price Here****/
        ...
        deleteExistingLogo : function(data, $el){
            ...
            //fix for recalculation once opened and closed popup and file selected, totals were wrong
            o.logoUpload($el);
        ...
        logoUpload : function($el){
           ...
           /* if(typeof $files == 'undefined'){
            ...
            //fix for recalculation once opened and closed popup and file selected, totals were wrong
            that.logoFilesTotal(0);
            that.priceForCustomDie(that.logoFilesTotal());

            $('.product-custom-option').each(function(){
                var currentVal = $(this).val();
                var checked = false;
                if($(this).parent('.control').prev().find('input[type="checkbox"]').length > 0){
                    checked = $(this).parent('.control').prev().find('input[type="checkbox"]').prop('checked');
                    currentVal = true;
                }
                if($(this).hasClass('sectionB') && currentVal != '' && !checked){
                    that.logoFilesTotal(that.logoFilesTotal()+that.options.sectionBPrice);
                }
                if($(this).hasClass('sectionC') && currentVal != '' && !checked){
                    that.logoFilesTotal(that.logoFilesTotal()+that.options.diePrice + that.options.sectionCPrice);
                    that.requireLogoOrCrest(true);
                }else{
                    that.requireLogoOrCrest(false);
        ...
        alreadyHaveLogoSection : function(data, $el){
           ...
           if(Object.keys(that.existingQuote).length > 0){
                ...
                //@TODO temp fix for getting minus price
                 // user already selected a file
                 if ($('#options_4969_file').get(0).files.length !== 0) {
                    if(elProp){
                        that.logoFilesTotal(that.logoFilesTotal() - that.options.diePrice - that.options.sectionCPrice );
